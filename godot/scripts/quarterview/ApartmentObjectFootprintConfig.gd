...
# Deprecated geometry compatibility fields. QuarterviewApartmentEnvironment never reads these;
# they remain serialized only for tested non-ROTATE_90/custom legacy previews.
@export var anchor_cell := Vector2i.ZERO
@export var size_cells := Vector2i.ZERO
@export var position_offset_px := Vector2.ZERO
@export var visual_size_px := Vector2.ZERO
@export var collision_shape_type: CollisionShapeType = CollisionShapeType.NONE
...
@export_range(0.0, 1.0, 0.01) var wall_position_ratio := 0.0
