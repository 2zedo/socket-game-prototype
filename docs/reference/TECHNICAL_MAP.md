...
P hit testing is separate from the gameplay interaction inventory. The four migrated objects are selectable only through their explicit SelectionPolygon; being inside InteractionPolygon, BodyPolygon, or visual bounds alone does not select them. The other fourteen Resource/fallback objects retain valid interaction, floor/collision, wall/parent anchor, then visual fallback ranking; ceiling and environment-only visuals remain last. Repeated clicks with the same candidate group cycle in ranked order. `DebugDetailPanel` additionally identifies `SCENE_NODE`, Inspector NodePath, visual/floor/selection sources, Selection size, BasePoint, TopPoint/height, UsePoint, and AttachmentSocket for migrated objects.
...
- `show_object_floor_footprints=true`, `show_object_collision_shapes=true`, and `show_object_interaction_areas=true` retain their existing controls. The migrated SelectionPolygon and BasePoint/TopPoint references form additional independent P channels and are always shown for those four objects while P is active.
