...
When `N` is enabled, the shell displays walkable floor cells, room areas, object-blocked cells, logical blocked wall edges, passable doorway edges, and a shell-only debug marker. Bed, fridge, NAVI, and NODE-17 blocked cells are derived from their Scene `Body/BodyPolygon` covering each floor-cell center; an optional future PlacementFootprint may override occupancy, but none of the current seven has one. The entrance door instead toggles its exact doorway edge between blocked while closed and passable while open, without floor occupancy. The other objects retain Resource/grid occupancy. N does not display object names, ids, sizes, interaction data, parent data, or `ObjectPlacementDebugLayer`. Walkable cells currently come from the visible living / work-power / bathroom floor cells, with future exception hooks in `_navigation_extra_walkable_cells()` and `_navigation_unwalkable_cells()`. Enabled wall segments create blocked edges; doorway units from `doorway_offset` / `doorway_width` are marked passable and excluded from blocked movement. `REVEALABLE`, `CUTAWAY_STUB`, and `HIDDEN_STUB` walls still count as logical blockers unless their edge is a doorway.
...
- `show_object_floor_footprints=true`, `show_object_collision_shapes=true`, and `show_object_interaction_areas=true` retain their existing controls. The migrated SelectionPolygon and BasePoint/TopPoint references form additional independent P channels and are always shown for those seven objects while P is active.
