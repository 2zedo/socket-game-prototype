...
- Main replacement must pass `docs/MAIN_REPLACEMENT_RISK_CHECKLIST.md` and follow `docs/MAIN_REPLACEMENT_WORK_PLAN.md` before any production entry scene or `project.godot` start-scene change.
...
- Yui quarterview spritesheet import is a separate visual migration step before replacing Main / DAY 1 player visuals.
