...
이번 작업은 문서화만 수행한다. 현재 메인 씬을 교체하거나, 쿼터뷰 prototype scene을 생성하거나, 기존 전력 / Phone / Result / Test Mode 흐름을 수정하지 않는다.
...
- 장기 방향 = 쿼터뷰 방 검토
- 이번 작업 = 문서화
- 이번 작업에서 구현하지 않음 = 코드 변경, scene 변경, Resource 변경, 에셋 추가, 쿼터뷰 prototype scene 생성
