...
- Hacking arena FX will use a separate `hack_fx_atlas.png` mapping workflow from room `qv_fx_atlas.png`, avatar, enemies, objects, projectiles, and arena tiles. Detailed mapping criteria are documented in `docs/HACK_FX_ATLAS_REGION_MAPPING.md`.
- Common UI components will use a separate `ui_common_atlas.png` mapping workflow from room, hacking, character, and FX assets. Detailed mapping criteria are documented in `docs/UI_COMMON_ATLAS_MAPPING.md`.
- Gameplay HUD components will use a separate `ui_hud_atlas.png` mapping workflow from common UI panels / buttons and from room / hacking visual assets. Detailed mapping criteria are documented in `docs/UI_HUD_ATLAS_MAPPING.md`.
- Phone UI components will use a separate `ui_phone_atlas.png` mapping workflow from common UI, HUD, outlet, result, and game visual assets. Detailed mapping criteria are documented in `docs/UI_PHONE_ATLAS_MAPPING.md`.
- Outlet UI components will use a separate `ui_outlet_atlas.png` mapping workflow from common UI, HUD, phone, result, and physical room cable assets. Detailed mapping criteria are documented in `docs/UI_OUTLET_ATLAS_MAPPING.md`.
- Result and log UI components will use a separate `ui_result_log_atlas.png` mapping workflow from common UI, HUD, phone, outlet, and game visual assets. Detailed mapping criteria are documented in `docs/UI_RESULT_LOG_ATLAS_MAPPING.md`.
- Dialogue UI components will use a separate `ui_dialogue_atlas.png` mapping workflow from common UI, HUD, phone, outlet, result / log, and game visual assets. Detailed mapping criteria are documented in `docs/UI_DIALOGUE_ATLAS_MAPPING.md`.
- Device icons will use a separate `ui_device_icons_atlas.png` mapping workflow from UI frames / cards, room object sprites, appliances, work devices, and cable sprites. Detailed mapping criteria are documented in `docs/UI_DEVICE_ICONS_ATLAS_MAPPING.md`.
