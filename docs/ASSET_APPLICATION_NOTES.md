...
- The completed back-facing frames are then scaled to `96%` and re-anchored to the same foot baseline, moving the visible top from `12` to `16` while keeping the visible bottom at `89`.
...
- `tools/process_yui_sprite_sheet.py`
  ...
  - Purpose: normalizes the non-back rows of the `yui-1` 4x4 RGBA sheet into matching `96x96` cells, keeps all frames on a shared foot baseline, mirrors the left row for the right-facing row to avoid the bad source crop, fits the back-facing row from intact fixed-grid source cells without redrawing or alpha cleanup, and scales only the completed back-facing frames to `96%` for runtime headroom.
