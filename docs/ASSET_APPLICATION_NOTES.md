...
This pass updates only the back-facing row of the generated active Yui walk sheet. It does not change the map background, object placement, UI panels, interaction logic, power logic, day/result systems, runtime display scale, or the front/left/right frame pixels.
...
- The back-facing row has a small repaired hair cap above the original bbox so the top of Yui's head no longer reads as clipped while the existing foot baseline remains unchanged.
...
- Up-direction headroom validation screenshots were captured at:
...
- `tools/process_yui_sprite_sheet.py`
  ...
  - Purpose: normalizes the `yui-1` 4x4 RGBA sheet into matching `96x96` cells, removes very low-alpha source noise, preserves crop padding for hair/shoes, keeps all frames on a shared foot baseline, mirrors the left row for the right-facing row to avoid the bad source crop, repairs only the back-facing hair top, and keeps Yui's source art as the player sprite.
