...
| Laptop | `Apartment.gd`, `Main.gd`, `SurvivalState.gd`, `DeviceDefinition` | `laptop`, `day1_action_key=laptop` | `laptop` | `laptop_job` | 현재는 active toggle. 장기적으로 work UI 또는 hacking entry 후보 | later sandbox | `sandbox_candidate` | `Laptop -> HackingActionPrototype` 연결은 아직 금지. 향후 Laptop mission flow는 `HackingMissionDefinition` Resource를 읽고, 성공 보상은 `GridCreditState`로 전달하는 방향을 별도 검증한다. |
...
| `OutletMode` | `Main.gd`, `OutletMode.gd` | `power_strip` interaction | `power` / `power_management` primary action | 6 | Sandbox에는 별도 mock Outlet panel이 연결됐다. 기존 Main `OutletMode`, `SurvivalState` slot state, Apartment wire overlay 연결은 아직 하지 않았다. |
| `PhoneUI` | `Main.gd`, `PhoneUI.gd` | `Tab` / `open_phone` | `Tab` 유지, 또는 `phone` primary action 후보 | 5 | Sandbox에는 별도 mock Phone panel이 연결됐다. 기존 Main `PhoneUI`와 실제 `SurvivalState` phone state 연결은 아직 하지 않았다. |
