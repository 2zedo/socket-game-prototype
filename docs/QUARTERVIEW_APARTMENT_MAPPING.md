...
| Laptop | `Apartment.gd`, `Main.gd`, `SurvivalState.gd`, `DeviceDefinition` | `laptop`, `day1_action_key=laptop` | `laptop` | `laptop_job` | 현재는 active toggle. 장기적으로 work UI 또는 hacking entry 후보 | later sandbox | `sandbox_candidate` | `Laptop -> HackingActionPrototype` 연결은 아직 금지. 향후 Laptop mission flow는 `HackingMissionDefinition` Resource를 읽는 방향으로 별도 검증한다. |
