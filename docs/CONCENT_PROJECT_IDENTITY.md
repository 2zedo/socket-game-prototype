...
- 중앙은 빈 이동 공간으로 둔다.
- 중앙 테이블, 작은 테이블, 큰 매트, 바닥 쿠션, 메모 공간은 제거 후보가 아니라 현재 방향상 제거한다.
- 왼쪽은 출입문과 전력 차단기 / 방 불 스위치 정도만 둔다.
- 신발, 외투, 발매트, 계단은 현재 방향에서 제거한다.
- 욕실은 눈에 띄지 않게 처리한다. 사용 가능한 넓은 공간처럼 보이면 안 된다.
- 침대는 왼쪽에서 중앙 쪽에 둔다.
- 침대 주변은 기계 장비가 아니라 유이의 성격과 바깥 세계 동경을 보여주는 공간이다.
- 오른쪽 창가 / 벽면에는 간결한 책상을 둔다.
- 책상 위에는 노트북, 최소 통신 장비, 짧은 메모 정도만 둔다.
- NODE-17, 신호 증폭기, 의미 모를 검은 박스, 과도한 오디오 장비는 현재 메인방 기본 배치에서 제거한다.
- 생활 가전 구역은 냉장고, 전자레인지, 싱크대 / 간단한 조리대 중심이다.
- 생활 가전 구역에는 해킹 장비를 섞지 않는다.
- 오른쪽 아래 구석에는 전력 장비를 둔다.
- 전력 장비는 대형 공개 서버랙이 아니라 관물대 / 금속 캐비닛처럼 숨겨진 느낌이다.
- 방 visual이 먼저 좋아 보여도 interaction / collision / prompt는 별도 invisible data로 분리한다.

유이의 자유 동경은 새 / 날개 직접 장식이 아니라 바다, 피서, 수평선, 해변 같은 오브젝트로 표현한다. 예시는 낡은 바다 포스터, 바다 화면보호기, 해변 광고 전단, 파란 조명, 물결 이미지, 인공 조개 장식이다. 단, 방 전체가 해변방처럼 보이면 안 된다.

재질 기준:

- 과도한 나무 가구는 피한다.
- 전체는 금속, 콘크리트, 합성 소재, 낡은 패널 중심이다.
- 침대 / 책상 일부는 어두운 합판이나 재활용 목재 정도는 가능하다.
- 감옥이나 연구소처럼 너무 차갑게 만들지 않는다.

## 5. 전력 장비 기준

전력 장비는 기존 멀티탭을 장기적으로 대체하는 핵심 시스템이다. 방 안에서는 compact 데이터센터 랙 / Molex / 모듈형 전력 보드 느낌을 참고하되, 대형 공개 서버랙이 아니라 오른쪽 아래 구석의 관물대 / 금속 캐비닛처럼 숨겨진 장치로 보이게 한다.

방 배치 기준:

- 문이 살짝 열려 안쪽의 모듈 보드와 케이블이 보이는 정도가 적절하다.
- UPS / 배터리팩을 별도 오브젝트로 늘어놓지 않는다.
- `Power` object는 나중에 power board close-up으로 진입할 하나의 명확한 오브젝트여야 한다.
