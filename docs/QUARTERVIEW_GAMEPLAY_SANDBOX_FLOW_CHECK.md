...
3. 열린 Outlet panel 처리
4. 열린 Phone panel 처리
5. 열린 End Day panel 처리
6. 열린 Interaction panel 처리
7. `Tab`: Phone panel toggle
8. `D`: debug overlay toggle
9. `E`: nearest interaction request
...
- Help label에는 `E`, `Tab`, `ESC`, `D`, `R`, `B` / `Backspace` 안내가 포함되어 있다.
