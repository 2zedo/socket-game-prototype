...
- Apartment Node migration stage 1 moves fridge, NAVI LINK, Power Board, and microwave exact ROTATE_90 position/visual/collision/interaction/UsePoint authority into editable Scene nodes. Resource entries now keep logical metadata only for those four, while the other fourteen objects retain Resource/fallback geometry. Hover/P/N/measurement and wall/parent following read the resolved Node data. Status: `KEEP_CANDIDATE`; Manual confirmation: `REQUIRED` for user Inspector tuning and later migration decisions.
