...
- Four stage-1 objects are Scene-Node-backed in ROTATE_90; the remaining object footprints stay Resource-backed with script fallback and additive Inspector custom entries preserved.
...
- Apartment shell data still uses the 18-object rotated-floorplan candidate; four objects now expose exact pixel geometry in the Scene Inspector while category/anchor semantics and the other fourteen footprints remain Resource-driven.
