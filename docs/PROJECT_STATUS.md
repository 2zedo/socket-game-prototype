...
3. `qv_fx_atlas.png` asset 제작과 region mapping prototype 적용을 별도 작업으로 진행한다. 시작 문서는 `docs/QV_FX_ATLAS_REGION_MAPPING.md`, `docs/QUARTERVIEW_STATIC_LIGHTING_OVERLAY_GUIDE.md`, and `docs/QV_WORK_DEVICES_ATLAS_REGION_MAPPING.md`이며, 완료 기준은 FX / body / static lighting category 분리, animated frame anchor stability, blend / alpha 후보, trigger metadata, prompt readability, and player / object readability가 확인되는 것이다.
