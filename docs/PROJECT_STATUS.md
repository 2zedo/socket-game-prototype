...
- Apartment editable object migration stage 2 now makes all seven direct-interaction objects Scene-Node authorities in ROTATE_90. Bed and NODE-17 gained independent Body/Selection/Interaction polygons, 2.5D markers, UsePoint, and AttachmentSocket; NODE-17's Resource-backed signal booster/cable children follow its socket. Entrance Door is a child of `EntranceWallParentAnchor`, has no floor occupancy, and toggles its exact doorway edge/body between closed-blocked and open-passable. The seven Resources retain logical metadata with geometry cleared; the other eleven environment objects remain Resource-backed. Godot MCP hierarchy, Inspector vertex edit/restore, selection/interaction/use independence, socket following, door state, and P+V runtime checks passed with zero current errors/warnings. Status: `KEEP_CANDIDATE`; Manual confirmation: `REQUIRED` before production wiring.
