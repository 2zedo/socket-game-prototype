...
- Five floor environment objects now use Scene-node geometry in `QuarterviewApartmentEnvironment`: sink counter, dining table, UPS, bathroom fixture, and shoes/slippers. Four blocking objects derive N/Playable occupancy from BodyPolygon; shoes/slippers has Selection only and never blocks. Microwave is a real child of the sink's `AttachmentSockets/MicrowaveSocket`, replacing the proxy without changing world geometry. Their Resources retain logical metadata with geometry cleared, while six environment/decoration objects remain Resource-backed. MCP confirmed Inspector root/polygon edit-and-restore, sink/socket child tracking, Environment propagation, P/N/V, and Playable movement with no new runtime errors. Status: `KEEP_CANDIDATE`; Manual confirmation: `COMPLETE` for this migration workflow; production promotion remains unapproved.
...
- Apartment editable object migration stage 2 makes all seven direct-interaction objects Scene-Node authorities in ROTATE_90. Bed and NODE-17 gained independent Body/Selection/Interaction polygons, 2.5D markers, UsePoint, and AttachmentSocket; NODE-17's Resource-backed signal booster/cable children follow its socket. Entrance Door has no floor occupancy and toggles its exact doorway edge/body between closed-blocked and open-passable; its retired proxy anchor was later replaced by the real EntranceWall Socket. The seven Resources retain logical metadata with geometry cleared. Five environment objects have since migrated too; six remain Resource-backed. Status: `KEEP_CANDIDATE`; Manual confirmation: `REQUIRED` before production wiring.
...
- All seven direct-interaction objects and five floor environment objects are Scene-Node-backed in ROTATE_90; six environment/decoration footprints stay Resource-backed with script fallback and additive Inspector custom entries preserved.
...
- Apartment shell data still uses the 18-object rotated-floorplan candidate; twelve objects expose exact pixel geometry in the Scene Inspector while category/anchor semantics remain Resource-driven and six environment/decoration footprints retain Resource geometry.
