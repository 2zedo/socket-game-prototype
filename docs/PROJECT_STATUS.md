...
- Apartment editable object stage 1 now uses the readable `Visual/Sprite2D/VisualPreview`, `Body/BodyPolygon`, `InteractionArea/InteractionPolygon`, sibling `UsePoint`, and `AttachmentSocket` contract for fridge, NAVI LINK, Power Board, and microwave. Interaction and access positions are independent; fridge/NAVI occupancy comes directly from BodyPolygon, while the other fourteen objects retain Resource/fallback geometry. Status: `KEEP_CANDIDATE`; Manual confirmation: `REQUIRED` for user Inspector tuning and later migration decisions.
