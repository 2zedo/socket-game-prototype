...
- The ROTATE_90 apartment Environment now stores its 98 floor cells in four editable TileMapLayers, room logic in four RoomArea polygons, ten walls in reusable WallSegment nodes, and five doors/windows in Opening Marker nodes. M/N/V/P and Playable movement read those Nodes; Environment startup no longer rebuilds floor diamonds or wall/opening visuals from script/Resource coordinates. Power Board and Entrance Door follow real wall Socket Markers. MCP confirmed the pre-run editor composition, edit/restore behavior, Shell P+V/M/N, Playable movement, and closed/open doorway transition. Eleven environment objects and the sink-parent proxy remain Resource-backed. Status: `KEEP_CANDIDATE`; Manual confirmation: `REQUIRED` before production promotion or final-art replacement.
