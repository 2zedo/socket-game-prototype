...
- Branch: `main`
- Current commit at task start: `4ef40d0`
- Phase: QV work devices atlas region mapping criteria
...
3. `qv_work_devices_atlas.png` asset 제작과 region mapping prototype 적용을 별도 작업으로 진행한다. 시작 문서는 `docs/QV_WORK_DEVICES_ATLAS_REGION_MAPPING.md`, `docs/ROOM_OBJECT_DEFINITION.md`, and `docs/HACKING_MISSION_DEFINITION.md`이며, 완료 기준은 work device / furniture / appliance / cable category 분리, stable state anchors, screen / signal FX separation, mission device key mapping, player scale, and prompt readability가 확인되는 것이다.
